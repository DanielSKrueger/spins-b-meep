...
class Sum(Function):
    ...
    def eval(self, inputs: List[flows.NumericFlow],
        ...
        arrays = [np.asarray(x.array) for x in inputs]
        
        returnArrays = arrays[1] + arrays[0]
        

        #flow.array = np.prod([x.array for x in inputs], axis=0)#old
        flow.array = returnArrays
...
class Product(Function):
    ...
    def eval(self, inputs: List[flows.NumericFlow]) -> flows.NumericFlow:
        ...
        returnArrays = arrays[1] * arrays[0]
        #flow.array = np.prod([x.array for x in inputs], axis=0)#old
        if(type(returnArrays) == np.float64):
            flow.array = np.atleast_1d(returnArrays)
        else:
            flow.array = returnArrays
